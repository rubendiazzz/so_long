...
void	ft_collect(t_game *game)
{
	if (game->map[game->image->player->instances->y
		/ PX_SIZE][game->image->player->instances->x / PX_SIZE] == COLLECTIBLE)
	{
		if (game->collected == game->data->collectibles)
			return ;
		ft_remove_collectible(game);
		game->map[game->image->player->instances->y
			/ PX_SIZE][game->image->player->instances->x / PX_SIZE] = EMPTY;
		game->collected++;
		if (game->collected == game->data->collectibles)
			game->image->exit->instances->enabled = false;
	}
	if ((game->map[game->image->player->instances->y
			/ PX_SIZE][game->image->player->instances->x / PX_SIZE] == EXIT)
...
void	ft_remove_collectible(t_game *game)
{
	...
	while (i < total)
	{
		if ((game->image->collectible->instances[i].y == game->image->player->instances->y)
			&& (game->image->collectible->instances[i].x == game->image->player->instances->x)
